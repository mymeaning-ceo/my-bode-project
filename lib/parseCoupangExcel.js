...
function parseCoupangExcel(filePath) {
  ...
  const findIndex = (names) =>
    names
      .map((n) => headers.findIndex((h) => String(h).trim().toLowerCase() === String(n).trim().toLowerCase()))
      .find((i) => i !== -1);
  ...
  const salesAmountIdx = findIndex(['Sales amount on the last 30 days', '30일 판매금액']);
  const salesCountIdx = findIndex(['Sales in the last 30 days', '30일 판매량']);
