document.addEventListener('DOMContentLoaded', () => {
  const table = $('#stockTable').DataTable({
    ...
    ajax: (data, cb) => {
      ...
      fetch(`/api/stock?page=${page}&limit=${data.length}&keyword=${encodeURIComponent(keyword)}`)
        .then(r => r.json())
        .then(json => {
          ...
          cb({ data: rows, recordsTotal: json.total, recordsFiltered: json.total });
        });
    }
  });

  $('#btnSearch').on('click', () => table.ajax.reload());
  $('#btnRefresh').on('click', () => { $('#keyword').val(''); table.ajax.reload(); });
  ...
  $('#btnDeleteAll').on('click', async () => {
    ...
    if (res.ok) { alert('삭제 완료'); table.ajax.reload(); }
    else alert('삭제 실패');
  });
});
