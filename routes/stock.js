...
router.get('/', async (req, res) => {
  try {
    ...
    const fields = selected && selected.length > 0 ? selected : (STOCK_COLUMNS || allFields);

    res.render('stock.ejs', { 결과: result, 필드: fields, 전체필드: allFields, 성공메시지: null });
  } catch (err) {
...
router.post('/upload', upload.single('excelFile'), async (req, res) => {
  try {
    ...
    const fields = STOCK_COLUMNS || allFields;
